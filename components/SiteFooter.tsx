...
export function SiteFooter() {
  ...
  const handleSubmitFeedback = async () => {
    ...
    if (trimmed.length < 8) {
      setNotice('Lutfen en az 8 karakterlik geri bildirim yazin.');
      return;
    }
    ...
    try {
      ...
      if (!response.ok || !payload.ok) {
        setNotice(payload.error || 'Geri bildirim gonderilemedi.');
        return;
      }
      ...
      setRating(5);
      setCategory('general');
      setNotice('Tesekkurler! Geri bildiriminiz kaydedildi.');
      setIsOpen(false);
    } catch {
      setNotice('Geri bildirim gonderilemedi. Lutfen tekrar deneyin.');
    } finally {
      setSubmitting(false);
    }
  };
      ...
      <div className="mx-auto w-full max-w-7xl px-4 py-5 sm:px-6">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <p className="text-xs text-slate-400 font-mono">
            Copyright © {year} Draw or Die. Tum haklari saklidir. Developed by{' '}
            <a
          ...
          <nav className="flex flex-wrap items-center gap-3 text-xs font-mono uppercase tracking-wider text-slate-400">
            ...
            >
              <MessageSquare size={12} /> Geri Bildirim
            </button>
        ...
        {isOpen && (
          <div className="mt-4 rounded-xl border border-white/10 bg-black/30 p-4">
            <h3 className="text-sm font-mono uppercase tracking-wider text-cyan-200">Hizli Geri Bildirim</h3>
            <p className="mt-1 text-xs text-slate-400">Bu mesaj dogrudan urun gelistirme listesine kaydedilir.</p>
            ...
            <div className="mt-3 grid gap-3 md:grid-cols-2">
              <label className="text-xs text-slate-400">
                ...
                >
                  ...
                  <option value="feature">Ozellik istegi</option>
                  <option value="ux">UX</option>
                  <option value="billing">Odeme</option>
                </select>
              </label>
            ...
            <label className="mt-3 block text-xs text-slate-400">
              ...
              <textarea
                ...
                rows={3}
                placeholder="Neyi iyilestirmemizi istersin?"
                className="mt-1 w-full rounded-lg border border-white/10 bg-black/40 px-3 py-2 text-sm text-white"
              />
            </label>
            ...
            <div className="mt-3 flex flex-wrap items-center justify-between gap-3">
              ...
              >
                <Send size={12} /> {submitting ? 'Gonderiliyor...' : 'Gonder'}
              </button>
            </div>
          </div>
        )}
