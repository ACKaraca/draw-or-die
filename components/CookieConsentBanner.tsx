...
export function CookieConsentBanner() {
  const [status, setStatus] = useState<'accepted' | 'rejected' | 'unset'>(() => {
      ...
      <div className="mx-auto flex w-full max-w-6xl flex-col gap-3 md:flex-row md:items-center md:justify-between">
        ...
        <div className="flex flex-shrink-0 gap-2">
          <button
            type="button"
            onClick={reject}
            className="rounded-md border border-white/25 px-4 py-2 text-xs font-mono uppercase tracking-wide text-white hover:bg-white/10"
          >
            Reddet
          </button>
          <button
            type="button"
            onClick={accept}
            className="rounded-md border border-emerald-400/40 bg-emerald-500/20 px-4 py-2 text-xs font-mono uppercase tracking-wide text-emerald-100 hover:bg-emerald-500/30"
          >
            Kabul Et
          </button>
        </div>
